    ...
    while (cliente.connected()) {
      if (cliente.available()) {
        ...
        if (c == '\n' && lineaenblanco) //Si la petición HTTP ha finalizado
        {
          ...
          cliente.println("<html style='background: #08cb61;'>");
          ...
          cliente.println("<input type=submit value=ON style='width:200px;height:75px;background-image:linear-gradient(blue, white); border-radius:6px; color:white;' onClick=location.href='./?LED=T\'>");
          cliente.println("<input type=submit value=OFF style='width:200px;height:75px;background-image:linear-gradient(black, white); border-radius:6px; color:white;' onClick=location.href='./?LED=F\'>");
          cliente.println("</center>");
